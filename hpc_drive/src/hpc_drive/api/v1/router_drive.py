...
def download_item(
    ...
    encoded_filename = quote(db_item.name)
    headers = {
...
def get_storage_usage(
    ...
    if current_user.is_unlimited_storage:
        quota = int(1e15) # 1 Petabyte (unlimited)
    elif current_user.custom_storage_quota_gb:
        quota = current_user.custom_storage_quota_gb * (1024**3)
    else:
        quota = settings_data.default_quota_gb * (1024**3)
        
    max_file_size = settings_data.max_upload_size_mb * (1024**2)
