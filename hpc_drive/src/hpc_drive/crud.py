...
def get_drive_item(
    ...
    if db_item.owner_id == user_id:
        return populate_stars_to_dicts(db, user_id, [db_item])[0]
...
def search_items(
    ...
    return populate_stars_to_dicts(db, user_id, db_items)


def admin_get_all_items(
    db: Session, skip: int = 0, limit: int = 100, search: str | None = None
) -> tuple[list[models.DriveItem], int]:
    ...
    if search:
        ...
        base_query = base_query.outerjoin(models.User, models.DriveItem.owner_id == models.User.user_id).filter(
            or_(
    ...
    items = base_query.order_by(models.DriveItem.created_at.desc()).offset(skip).limit(limit).all()
